...
autocmd("LspAttach", {
  ...
  callback = function(event)
    vim.keymap.set("n", "K", vim.lsp.buf.hover, {desc = "Documentation", buffer = event.buf })
    vim.keymap.set("n", "gd", vim.lsp.buf.definition, {desc = "Goto Definition", buffer = event.buf })
    vim.keymap.set("n", "gD", vim.lsp.buf.declaration, {desc = "Goto Implementation", buffer = event.buf})
    vim.keymap.set("n", "gi", vim.lsp.buf.implementation, {desc ="Goto Implementation", buffer = event.buf})
    vim.keymap.set("n", "go", vim.lsp.buf.type_definition, {desc = "Goto Overlad", buffer = event.buf})
    vim.keymap.set("n", "gr", vim.lsp.buf.references, {desc = "Goto References", buffer = event.buf})
    vim.keymap.set("n", "gs", vim.lsp.buf.signature_help, {desc = "Goto Signature Help", buffer = event.buf})
    vim.keymap.set("n", "gl", vim.diagnostic.open_float, {desc = "Goto Line Diagnostics", buffer = event.buf})
    vim.keymap.set("n", "[d", vim.diagnostic.goto_prev, {desc = "Goto previous Diagnostic", buffer = event.buf})
    vim.keymap.set("n", "]d", vim.diagnostic.goto_next, {desc = "Goto next Diagnostic", buffer = event.buf})
    vim.keymap.set("n", "ga", vim.lsp.buf.code_action, {desc = "Goto code Action", buffer = event.buf})
  end,
})
